...
public class Punto_2 {
    public static void main(String[] args) {
        ...
        int[][] matriz3 = multiplicarMatriz(matriz1, matriz2);
    ...
    public static int[][] multiplicarMatriz(int[][] matrizUno, int [][] matrizDos){
        int [][] matrizMult = crearMatriz(matrizUno.length, matrizUno[0].length);
        for (int i = 0; i < matrizUno.length; i += 1){
            for (int j = 0; j < matrizUno[i].length; j += 1){
                matrizMult[i][j] = matrizUno[i][j] * matrizDos[i][j];
            }
        }
        return matrizMult;
    }
